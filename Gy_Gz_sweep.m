% En este codigo realiza un barrido en el espacio de parametros y genera un
% diagrama de bifurcación del modelo con cambio fenotipico
...
for i=1:m1
    for j=1:m2
        P=[r0 s0 K n c d1 d2 b theta1 theta2 theta3 theta4 p q0 e cx cy cz G1 G2(i) G3(j)];

        % Simulación del Modelo
        options = odeset('RelTol', 1e-8, 'AbsTol', 1e-10);
        [tv,Yv] = ode45(@(t,Y) vivi(t,Y,P),[0 20000],x0);

        % Determinar el color según las condiciones dadas
        if Yv(end,1) > 0.00001 && Yv(end,2) > 0.00001 && Yv(end,3) > 0.00001 %coexistencia
            R1(i,j) = 1; %Es azul
        elseif Yv(end,1) > 0.00001 && Yv(end,2) < 0.00001 && Yv(end,3) > 0.00001 
            R1(i,j) = 2; % Exz amarillo
        elseif Yv(end,1)  < 0.00001  && Yv(end,2)  < 0.00001  && Yv(end,3) > 0.00001 
            R1(i,j) = 3; %Ez rojo
        else
            R1(i,j) = 4; %Exy verde
